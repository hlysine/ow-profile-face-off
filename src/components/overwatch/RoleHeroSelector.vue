...
export type SelectionType = 'role' | 'hero' | 'all-hero' | 'role-hero' | 'everything';
...
<template>
  <div class="w-full flex flex-col gap-4 items-start">
    <div class="flex w-full gap-4 items-center">
      <span class="text-3xl text-slate-400 flex-1">Hero Selection</span>
      <CustomButton :type="type === 'all-hero' ? 'primary' : 'flat'" class="text-2xl" @click="select('all-hero', '')"
        >All Heroes
      </CustomButton>
    </div>
    <div v-for="role in roles" :key="role.key" class="flex gap-4 items-center">
      ...
      <div class="shrink-0 w-[1px] h-20 mx-4 border border-slate-600"></div>
